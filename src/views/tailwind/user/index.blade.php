...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h4 class="text-xl font-bold text-gray-900 dark:text-gray-100">User and Right Management</h4>
        @if (Route::has('useraccess.user.create') && Route::has('useraccess.user.store'))
            <a href="{{ route('useraccess.user.create') }}" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md text-sm font-medium transition">Create User</a>
        @endif
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden p-6">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 display" id="usersTable">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700 text-sm text-gray-700 dark:text-gray-300">
                    <?php foreach ($users as $user) { ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors">
                            <td class="px-4 py-3 whitespace-nowrap"><?php echo $user['id']; ?> </td>
                            <td class="px-4 py-3 whitespace-nowrap font-medium text-gray-900 dark:text-gray-100"><?php echo $user['name']; ?></td>
                            <td class="px-4 py-3 whitespace-nowrap"><?php echo $user['role']; ?></td>
                            <td class="px-4 py-3 whitespace-nowrap"><?php echo $user['email']; ?></td>
                            <td class="px-4 py-3 whitespace-nowrap flex items-center space-x-2">
                                @if (Route::has('useraccess.user.edit') && Route::has('useraccess.user.update'))
                                    <a href="<?= route("useraccess.user.edit", $user['id']) ?>" class="p-1.5 bg-yellow-500 hover:bg-yellow-600 text-white rounded transition text-xs flex items-center justify-center w-8 h-8"><i class="fa fa-pencil"></i></a>
                                @endif
                                @if (Route::has('useraccess.user.delete'))
                                    <form action="{{ route('useraccess.user.delete', $user['id']) }}" method="POST" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this user?');">
                                        @csrf @method('DELETE')
